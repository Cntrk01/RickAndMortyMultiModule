        ...
        characterDetailsRepository
            ...
            .onSuccess { character ->
                val dataPoints = buildList {
                    add(DataPoint(title = "Last know location",character.location.name))
                    add(DataPoint(title = "Species",character.species))
                    add(DataPoint(title = "Gender",character.gender.displayName))
                    character.type.takeIf { it.isNotEmpty() }?.let { type ->
                        add(DataPoint(title = "Type",type))
                    }
                    add(DataPoint(title = "Origin",character.origin.name))
                    add(DataPoint(title = "Episode count",character.episodeIds.size.toString()))
                }
